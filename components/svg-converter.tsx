import { Download, FileImage, CheckCircle, Loader2, RefreshCw } from "lucide-react"
...
export function SVGConverter({ 
  ...
  const getConvertButtonState = useCallback(() => {
    ...
    const isDisabled = !imageUrl || isGenerating || isConverting || convertAttemptedRef.current
  ...
  const getHelperText = useCallback(() => {
    ...
    if (svgResult) {
      return "Conversion completed! You can convert again or download the SVG."
    }
    
    return "Click to convert the generated image to SVG format"
  }, [isGenerating, imageUrl, isConverting, error, svgResult])

  const { isDisabled: isConvertDisabled, variant: convertVariant } = getConvertButtonState()
          ...
          {isConverting && (
            <div className="flex items-center justify-center py-20">
              <div className="text-center space-y-4">
                ...
                <p className="text-sm text-muted-foreground">Converting to SVG...</p>
              </div>
            </div>
          )}
              ...
              <div className="space-y-3">
                <div className="text-xs text-muted-foreground space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="uppercase">{svgResult.content_type.split('/')[1] || 'SVG'}</span>
                  </div>
                  <div className="flex justify-between">
